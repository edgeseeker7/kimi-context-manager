...
export function activeSessionKey() {
  ...
  // Pointer for the kernel-side /reset checkpoint: which sessionKey owns the
  // pins/notes of the session currently being worked on.
  const pointer = join(KIMI_HOME, 'context-manager', 'current.json');
  mkdirSync(dirname(pointer), { recursive: true });
  writeFileSync(pointer, JSON.stringify({ sessionKey: key, wire, ts: Date.now() }));
  return key;
}
